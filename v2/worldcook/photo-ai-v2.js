/* WorldCook Vision v5 — live camera, selectable localized candidates, dual on-device recognition. */
(function(){
...
ko:{load:"재료 탐지 모델을 불러오는 중입니다. 최초 1회만 잠시 걸릴 수 있습니다.",detect:"사진 속 재료를 찾는 중입니다…",fallback:"고정밀 분석을 사용할 수 없어 호환 분석을 실행합니다…",retake:"📸 사진 다시 찍기",source:"기기 내 이중 분석",low:"신뢰할 만한 결과가 없습니다. 재료를 가까이에서 밝게 다시 찍거나 직접 추가하세요."},
...
ja:{load:"食材検出モデルを読み込み中です。初回のみ少し時間がかかる場合があります。",detect:"写真の食材を検出中です…",fallback:"高精度分析を利用できないため互換スキャンを実行します…",retake:"📸 写真を撮り直す",source:"端末内2段階分析",low:"信頼できる結果がありません。食材を明るい場所で近くから撮り直すか、手動で追加してください。"},
zh:{load:"正在加载食材检测模型，首次使用可能需要一点时间。",detect:"正在识别照片中的食材…",fallback:"高精度分析不可用，正在运行兼容扫描…",retake:"📸 重新拍摄",source:"设备端双重分析",low:"没有足够可靠的结果。请在明亮环境中近距离重拍，或手动添加食材。"}
};
const SCAN_I18N={
...
async function loadModels(){
 if(!objectDetectorPromise){
  ...
  activeDetector=useGPU?"OWL-ViT WebGPU":"OWL-ViT";
  objectDetectorPromise=hf.pipeline("zero-shot-object-detection","Xenova/owlvit-base-patch32",useGPU?{device:"webgpu",dtype:"q4"}:{dtype:"q8"});
 }
...
window.analyzePhoto=async function(){
 ...
 try{
  const detector=await loadModels();setStatus(t.detect);
  const detection=await detector($("#photoPreview").src,labels,{top_k:50,threshold:.055});
  const best={};
  for(const x of detection){const n=keyByLabel.get(x.label)||mapLabel(x.label);if(n)best[n]=Math.max(best[n]||0,+x.score||0)}
  detected=Object.entries(best).map(([n,score])=>({n,score})).sort((a,b)=>b.score-a.score).slice(0,18);
  ...
  console.warn("WorldCook Vision v5 fallback",err);objectDetectorPromise=null;setStatus(t.fallback);btn.disabled=false;return legacyAnalyze();
